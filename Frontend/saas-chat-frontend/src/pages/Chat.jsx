...
const Chat = () => {
  ...
  // Helper — handle both old and new field names
  ...
  const getUserId = (msg) => msg.userId || msg.sender || "";
  const getMyId = () => user?.id || user?.user?.id || user?._id || "";
  ...
  const sendMessage = () => {
    ...
    socket.emit("send_message", {
      userId: user?.id || user?.user?.id || user?._id,
      message: newMessage.trim(), // ✅ "message" not "content"
    });
    setNewMessage("");
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.messagesContainer}>
        {messages.length === 0 && (
          <div style={styles.noMessages}>No messages yet. Be the first to say hi! 👋</div>
        )}
        {messages.map((msg, index) => (
          <div key={msg._id || index} style={{
            ...styles.message,
            alignSelf: String(getUserId(msg)) === String(getMyId()) ? "flex-end" : "flex-start",
            background: String(getUserId(msg)) === String(getMyId()) ? "#6c63ff" : "#fff",
            color: String(getUserId(msg)) === String(getMyId()) ? "#fff" : "#333",
          }}>
      ...
      {!isPro && (
        <div style={styles.upgradeBanner}>
          ...
          <span style={styles.upgradeLink} onClick={() => navigate("/dashboard")}>
            Upgrade to Pro
          </span>{" "}
          to send messages
        </div>
      ...
      <div style={styles.inputContainer}>
        <input
          style={{ ...styles.input, opacity: isPro ? 1 : 0.5 }}
          type="text"
        ...
        />
        <button
          style={{ ...styles.sendBtn, opacity: isPro ? 1 : 0.5 }}
          onClick={sendMessage}
          disabled={!isPro}
        >
          Send
...
const styles = {
  ...
  planBadge: { padding: "4px 12px", borderRadius: "20px", color: "#fff", fontSize: "13px" },
  ...
  upgradeLink: { color: "#6c63ff", cursor: "pointer", fontWeight: "bold" },
